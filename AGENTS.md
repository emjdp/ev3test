...
- **BACK 버튼은 프로그램 입력으로 할당하지 않는다.** ev3dev 기본 종료 동작으로 남기고,
  정지는 네트워크 `stop`/키보드 인터럽트/필요 시 별도 안전 입력으로 처리한다.
...
- **BACK 버튼을 프로그램 제어 입력으로 쓰지 않는다.** 브릭에서 프로그램 종료 버튼처럼
  동작하므로, 앞으로 새 코드는 BACK 에 stop/skip/abort 기능을 별도로 배정하지 않는다.
...
## 3. Git 규약 (원격 공유: GitHub `origin`)

원격 `origin`(`https://github.com/emjdp/ev3test.git`)에 **팀원과 공유**한다. 각 에이전트는
*자기가 한 변경을 직접 커밋*하고, 의미 있는 단위가 끝나면 `master` 를 push 한다.

> 정책 변경(2026-06-30): 초기에는 "원격 없음(로컬 추적용)"이었으나 팀 공유를 시작하며 GitHub
> `origin` 을 연결했다. 옛 커밋 메시지/문서에 남은 "원격 없음" 표현은 이 시점 이전 기록이다.

- 의미 있는 변경 단위로 **자주, 작게** 커밋한다.
- **push**: 작업 단위를 커밋한 뒤 `git push`(또는 `git push origin master`)로 공유한다.
  push 는 외부 공개이므로, 실기 미검증/실험 커밋을 올릴 때는 그 사실을 메시지에 분명히 남긴다.
  강제 push(`--force`)는 공유 이력을 깨므로 쓰지 않는다(되돌리기는 새 커밋으로).
- 시작 전 `git pull` 로 팀원 변경을 먼저 받는다(충돌 나면 작은 단위로 해소).
