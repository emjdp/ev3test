...
추가 병합(2026-07-07, codex): `inchul` 브랜치의 안정화 실험판
`stages/run_maze_v11.py` 를 master 로 가져왔다. v4 기반 단일 실행 파일이며
기본값(`base_speed=16`, `kp=0.2`, `turn_speed=6`, `node_confirm_ms=90`,
`left_th_steer=66`, `right_th_steer=63`, `node_advance_mm=20`,
`goal_advance_mm=20`, `turn_90_factor=0.66`, `turn_180_factor=0.71`,
`grab_dist_cm=6`, `grip_speed=50`, `left_th_node=18`, `right_th_node=14`),
D항 `0.05`, 후보 지점 PD-off 저속 직진 확정, 색마커 즉시 U턴/부저 2회 처리를
포함한다. **PC 검증 및 실기 검증 필요.**

추가 병합(2026-07-07, codex): `inchul` 브랜치의 v12 안정화 실험판
`stages/run_maze_v12.py` 를 master 로 가져왔다(`PROGRESS.md` 는 브랜치 것을 가져오지
않고 master 기준으로만 기록). v5 reset 세션 구조를 기준으로 v11 안정화 아이디어
(D항 `0.05`, 후보 지점 PD-off 저속 직진 확정, 빨강/노랑/초록 마커 즉시 U턴/부저
2회)를 다시 반영한 단일 실행 파일이다. PC 검증(`py_compile`) 완료. **실기 검증 필요.**

대시보드 개선(2026-07-07, codex): `run_maze_v11` 실기 중 판단 흐름을 쉽게 보기 위해
`tools/dashboard.py` 에 `move log (from start)` 섹션을 추가했다. `NODE_CONFIRMED.bits` 와
이후 선택 이벤트(`BRANCH_PROBE`/`RETURN_STEP` 등)의 `move` 를 묶어 `L(110)`/`R(011)`/
`S(111)` 처럼 표시하고, 시작·홈·마커 U턴은 `START(yellow)`/`HOME(yellow)`/`U(red)` 로
보인다. 긴 주행에서 초반 이벤트가 바로 밀려나지 않도록 `DecisionLog` 와
`telemetry_watcher` 의 최근 이벤트 보관량을 20→120개로 늘렸다. PC 검증:
`py_compile`, `tests/test_dashboard_logic.py`, `tests/test_stage3_logic.py` 통과.
**실기에서는 v11 실행 중 watcher+dashboard 화면에서 move log 누적 표시만 확인하면 된다.**

신규(2026-07-07, claude): **`stages/frun_v1.py` — 편도 좌수법 재작성판.** v4/v5 의
work_id/pending/BACKTRACK DFS 상태머신이 실기에서 상태-실위치 desync 로 같은 구간에
갇히는 문제, 그리고 111 분기에 비스듬히 진입하면 confirm 창에서 bits 가 110↔011 로
깜빡여 후보가 리셋되고 111 을 놓치는 문제(사용자 보고)를 겨냥해 판단층을 갈아엎었다.
- **무상태 좌수법**: 분기에서 열린 출구 L>S>R, 없으면 U턴. 상태머신 없음 — 트리 코스에서
  경계 순회로 완주 보장, 오판해도 자기복구. 111 최초 방문만 예외(D형 직진 우선),
  T자(직진 막힘)는 `peek_backup_mm` 후진 복귀 후 L. 최초방문 판정용 슬림 지도(FrunMap)는
  참고용 — mismatch 시 trusted=False 로 D형 예외만 꺼지고 주행은 계속.
- **111 OR-latch**: confirm 창 동안 CANDIDATES 내 bits 를 비트별 OR 합산(110|011=111).
  `left_th_node`/`right_th_node` 를 고정 상수에서 라이브 params 로 개방(총 15개).
- **trail 표시**: 모든 판단을 `L(110) R(011) S(111) U(red) G(goal)` 식으로 누적,
  telemetry `trail` 필드(최근 20개)로 대시보드에 상시 표시(`tools/dashboard.py` 에
  trail 줄 추가 — 왼쪽 잘라 최신이 보이게). 도착/정지 시 `RUN_TRAIL` 로 전체 기록.
- v5 세션 루프/reset/pause 유지, 편도라 HOME/복귀 없음(초록 즉시 그리퍼 오픈+정지).
- PC 검증: `py_compile`, `tests/test_frun_v1_logic.py` 15개 통과(잎 U턴 phantom 노드
  방지 회귀 테스트 포함). DECISIONS.md 카탈로그에 신설 reason_code 6줄 추가.
  **실기 검증 필요.**
...
- [ ] **frun_v1 편도 좌수법 실기 검증** — `stages/frun_v1.py` 를 브릭에서 실행해
      ...
      111 인식 실패 시 `left_th_node`/`right_th_node`/`node_confirm_ms` 를 **한 번에 하나만**
      조정한다(`NODE_CANDIDATE_LATCH` 의 bits_raw vs bits_latched 로 OR-latch 효과 확인).
...
- [ ] **run_maze_v11 안정화 실험판 검증** — `inchul` 브랜치에서 가져온
      `stages/run_maze_v11.py` 를 PC 문법 점검 후 브릭에서 실행해 후보 지점 저속 직진 확정,
      색마커 즉시 U턴/부저 2회, 재포착 후 위빙 완화가 의도대로 동작하는지 확인한다.
      실패 시 한 번에 값 하나만 조정하고 기록한다. 대시보드 `move log (from start)` 에
      `START(yellow)` 뒤로 `L(110)`/`R(011)`/`S(111)`/`U(red)` 같은 누적 기록이 뜨는지도
      함께 확인한다.
- [ ] **run_maze_v12 안정화 실험판 검증** — `inchul` 브랜치에서 가져온
      `stages/run_maze_v12.py` 를 브릭에서 실행해 v5 reset 세션 구조가 유지되는지와
      후보 지점 저속 직진 확정, 색마커 즉시 U턴/부저 2회가 의도대로 동작하는지 확인한다.
      실패 시 한 번에 값 하나만 조정하고 기록한다.
...
- **원인**: run8 조향은 좌우 반사광 차 PID 인데, 가로선을 한쪽 센서가 먼저 스치면
  bits 노드 후보 확정(임계 35/30) **전** 하강 구간(원시값 74→15~40)에서 PID 가 그
  차이를 라인 이탈로 오해. D항 스파이크까지 겹쳐 turn 이 한계(±16)로 치솟아 거의
  피벗 — 그 틀어진 자세로 confirm_node 직진 크리프 진입. confirm 취소 후 디바운스
  150ms 동안 센서가 가로선 위에 남으면 같은 급조향 반복(위빙).
- **반영(`ev3final/stages/final_run8.py`, §변경 F — 원격 fddadd6 이 E(on_line)를
  선점해 리베이스하며 F 로 재부여)**: 반사광 < 노드임계+12(47/42) 이면
  가로선 접근으로 보고 조향 off + SLOW_SPEED 직진으로 자세 유지 — confirm_node 확정
  또는 값 회복까지. 700ms 초과 지속이면 정렬 불량(세로선 위)이므로 PID 복귀.
  근거: 일반주행 하위 1% 실측 51~58 이라 47/42 아래는 사실상 가로선뿐(12:01 로그).
  라이브 파라미터 추가 없음(상수 `NODE_GUARD_MARGIN`/`NODE_GUARD_MAX_MS`).
- **PC 검증**: py_compile + `tests/test_run_maze_v4_logic.py` 11개 통과. **실기 검증
  필요** — 교차로 직진 진입 시 `NODE_GUARD STRAIGHT_HOLD` 로그 후 자세 유지 확인.
  run10 은 중앙센서 엣지 조향이라 이 문제 구조가 아님(미반영).
