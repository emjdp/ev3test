...
노드 후 엔코더 전진 `ADVANCE_SPEED=11` 로 낮췄다.**
...
| Stage 5 통합(트레이싱+회전) | 🟡 진행 중 | 2026-07-06 `stages/stage5_integration.py` 1차 구현. 사진 미로 직접 도착 경로 추정 기본값 `R L L R L L R L S S L R L L R L S S R L R R L L L`; `--seq` 로 현장 수정 가능. Stage 4 저장 색 튜닝값으로 보라/빨강 마커를 읽고, 확정 시 자동 U턴한다. 브랜치 씹힘 대응으로 `base_speed=13`, Stage 5 전용 `ADVANCE_SPEED=11`. 실기 검증 필요 |
| Stage 6 탐색/복귀 | ⬜ 시작 전 | |
| Stage 7 물체 집기 | ⬜ 시작 전 | |
