...
## 2026-07-07 run_maze_v4 가져오기 (Agent: codex)

- `origin/HoJeongDu` 브랜치의 `stages/run_maze_v4.py`를 `inchul` 브랜치로 가져왔다.
...
자동 회전하지 않고 사진 기반 기본 시퀀스(`R L L R L L R L S S L R L L R L S S R L R R L L L`, `--seq` 로 교체 가능)를 소비한다. 실기에서
교차점 수/시퀀스 검증이 아직 필요하다. 2026-07-06 현재 Stage 4 의 보라/빨강 색 마커 감지 후
자동 U턴도 Stage 5 에 반영했다. 브랜치 씹힘 대응으로 Stage 5 주행 `base_speed=13`,
노드 후 엔코더 전진 `ADVANCE_SPEED=11` 로 낮췄다. 2026-07-07 노드 후보 감지 순간부터
`node_confirm_count` 진행률에 맞춰 선형 감속하는 로직도 추가했다.**
...
| Stage 5 통합(트레이싱+회전) | 🟡 진행 중 | 2026-07-06 `stages/stage5_integration.py` 1차 구현. 사진 미로 직접 도착 경로 추정 기본값 `R L L R L L R L S S L R L L R L S S R L R R L L L`; `--seq` 로 현장 수정 가능. Stage 4 저장 색 튜닝값으로 보라/빨강 마커를 읽고, 확정 시 자동 U턴한다. 브랜치 씹힘 대응으로 `base_speed=13`, Stage 5 전용 `ADVANCE_SPEED=11`, 노드 후보 선형 감속 적용. 실기 검증 필요 |
| Stage 6 탐색/복귀 | ⬜ 시작 전 | |
| Stage 7 물체 집기 | ⬜ 시작 전 | |
...
- [ ] **Stage 5 실기 검증** — `python3 stages/stage5_integration.py --seq "R L L R L L R L S S L R L L R L S S R L R R L L L"` 기본 경로가
      사진 미로의 실제 교차점 순서와 맞는지 확인한다. 한 노드가 어긋나면 코드 수정 대신 `--seq`
      만 바꿔 재실행한다. Stage 5 는 색 마커를 `COLOR_READ` 로 기록하고, Stage 4 처럼
      `MARKER_UTURN` 후 자동 U턴한다.
...
### 2026-07-07 — Stage 5 노드 후보 감지 시 선형 감속 추가 (Agent: codex)
- **요청**: 노드 판별 안정성을 위해 노드처럼 보이는 순간 일정하게 감속해 속도를 0으로 만드는 기능 추가.
- **구현**: `node_approach_speed_scale()` 순수 함수를 추가해 노드 후보가 없으면 1.0, 후보가 보이면 `node_confirm_count` 진행률에 맞춰 선형 감속한다. 기본 `node_confirm_count=2` 에서는 첫 후보 프레임 50%, 다음 확정 프레임 정지 흐름이다.
- **적용 위치**: Stage 5 라인추종 루프에서 `pd_step()` 으로 계산한 좌우 속도에 감속 스케일을 곱한다. 확정 로직과 시퀀스 소비는 그대로 둔다.
- **테스트**: `tests/test_stage5_logic.py` 에 감속 스케일 순수 함수 테스트 추가. 로컬 PC Python 실행기 여부에 따라 실행 가능하면 확인 필요.

### 2026-07-06 — Stage 5 브랜치 씹힘 대응 속도 하향 (Agent: codex)
- **요청**: 브랜치를 한 번씩 씹는 증상 대응으로 Stage 5 속도 하향.
- **변경**: `stages/stage5_integration.py` 의 기본 `base_speed` 를 17→13 으로 낮췄다.
- **변경**: Stage 3 Done 파일의 `ADVANCE_SPEED=15` 는 건드리지 않고, Stage 5 파일에 전용 `ADVANCE_SPEED=11` 을 두어 `S` 직진 토큰과 좌/우 회전 전 엔코더 전진 보정에만 적용한다.
- **실기 필요**: 같은 기본 시퀀스로 재주행해서 `NODE_CONFIRMED` 누락이 줄었는지 확인한다. 아직 씹히면 다음 한 변수 후보는 `node_confirm_count` 이다.

### 2026-07-06 — Stage 5 색 마커 자동 U턴 반영 (Agent: codex)
- **요청**: Stage 4 에 있던 “보라색/색 마커를 만나면 U턴” 동작이 Stage 5 에 빠진 것 같으니
  Stage 5 에도 반영.
- **구현**: `stages/stage5_integration.py` 의 자동 색 감지 경로에서 마커가 확정되면
  Stage 4 의 `run_marker_uturn` 을 호출해 `MARKER_UTURN` 로그를 남기고 180도 회전하도록 했다.
- **색 튜닝값**: Stage 5 가 Stage 4 의 저장 파일(`config/stage4_clolor_reflected.json`)을 읽어
  보라/빨강 후보 범위와 RGB 비율 확정값을 기본으로 쓰게 했다. Stage 5 라이브 params 는 기존
  6개를 유지한다.
- **테스트**: `python3 -m py_compile stages/stage5_integration.py tests/test_stage5_logic.py`,
  `python3 tests/test_stage5_logic.py` 통과. 실기 검증 필요.
...
- **기본 시퀀스**: 사진상 직접 도착 경로 추정값 `R L L R L L R L S S L R L L R L S S R L R R L L L`. 첫 실기에서 교차점 수가 다르면 코드 수정 없이 `--seq "..."` 로 바꿔 재실행한다.
- **색 처리(당시 1차 구현)**: Stage 4 reflected 색 읽기는 유지하지만, 이 시점에는 색 인식이 자동 U턴을 트리거하지 않고 `COLOR_READ` 로 기록만 했다. 이후 위 작업 로그처럼 `MARKER_UTURN` 자동 U턴을 반영했다.
