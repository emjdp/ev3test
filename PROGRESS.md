...
- **실기 증상**: 라인트레이싱 중 조금 틀어지면 자꾸 180도 유턴. 원인 분석에서
  ...
  ② 검정 라인의 파랑 오인식(BLUE_REVISIT 즉시 유턴) 두 후보 제시 — 사용자가
  ②(파랑 오인식)로 판단. EV3 컬러센서의 검정↔파랑 혼동은 알려진 특성.
- **반영 (`stages/run_maze_v2.py`)**: `COL_VISIT=빨강(5)`, `COL_GOAL=초록(3)` 상수
  신설 후 방문/도착 판정 교체(빨강은 검정과 혼동이 가장 적음 → 오인식 잦은
  방문 마커에 배정, 도착은 초록으로 이동). 유턴 rule 은 RED_REVISIT 로 갱신.
  코드 외 변경 없음(한 번에 변수 하나 — 000 복구 체인은 이번에 안 건드림).
- **★ 물리 코스 교체 필요**: 방문 노드 테이프 파랑→빨강, 도착 테이프 빨강→초록.
  (노랑 출발은 그대로.)
- **검증**: py_compile + v2/v1 판단층 테스트 통과. **실기 검증 필요** — 재주행에서
  가짜 유턴이 계속되면 남는 후보는 ①(000 복구 체인)이므로 유턴 직전 이벤트
  (LINE_LOST/DEAD_END 계열인지)를 로그로 확인할 것. 초록 도착 오인식이 보이면
  도착도 연속확정(stage4v2 color_confirm_count 방식) 도입 검토.
...
- **실기 증상**: 라인에 똑바로 올라타 출발하면 괜찮은데, 비뚤어져 보정이 시작되면
  ...
  ① v1 `line_error` 가 계단식(0/±0.5/±1.0/±2.0)+D항 없음 → 조향 점프/진동,
  ② 중앙 색상값이 조향에 들어가는데 라인 경계에서 색상모드 판독 불안정,
  ③ 휘청일 때 사이드 센서가 라인을 깊게 가로질러 가짜 노드 bits/000 confirm.
- **반영 (`stages/run_maze_v2.py` 신규 — v1 은 수정하지 않음)**:
  - 조향을 stage3v2 확정 PD(`PdController` import, kp 0.22/KD 0.05/TURN_LIMIT 16)로
    교체. 중앙 색상은 조향에서 제외(노드/마커 판정 전용 — stage4v2 와 동일 원리).
    중앙 상시 컬러모드(모드 전환 0회)는 유지.
  - 000 가드 `lost_candidate_blocked`: 직전 |turn| > `LOST_GUARD_TURN`(8.0) 이면
    (0,0,0) 을 노드 후보로 안 잡음 — 보정 중 순간 이탈의 막다른길 오검출 차단.
    가드는 000 에만 걸고 진짜 커브/분기 bits(사이드 20/18 미만)는 막지 않는다.
  - 회전/후진/일시정지 등 주행 단절마다 `pd.reset()`(D항 스파이크 방지).
  - 탐색 로직(우>좌>직+직전 분기 기억)/000 후진 복구/그리퍼/초음파는 v1 에서
    import 재사용(복붙 없음). 라이브 params 는 follow_gain → kp 교체(9개 유지),
    base_speed 시드 17(PD 확정 조합).
- **검증**: `py_compile` 통과, `tests/test_run_maze_v2_logic.py` 신규(000 가드,
  pd.step 중앙 raw 무시 회귀, 방향/클램프, params 메타) + 기존
  `tests/test_run_maze_logic.py` 회귀 통과. **실기 검증 필요**(kp/base_speed 조합,
  LOST_GUARD_TURN, 커브 인식 타이밍).
- **다음**: 브릭에 배포해 v1 과 동일 코스로 비교. 발진이 남으면 kp ↓ 또는
  base_speed ↓, 커브에서 늦게 서면 NODE_CONFIRM_MS/LOST_GUARD_TURN 재검토.
