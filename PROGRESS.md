...
| Stage 5 통합(트레이싱+회전) | 🟡 진행 중 | 2026-07-06 `stages/stage5_integration.py` 1차 구현. 사진 미로 직접 도착 경로 추정 기본값 `R L L R L L R L S S L R L L R L S S R L R R L L L`; `--seq` 로 현장 수정 가능. 색 마커는 읽고 기록만 하며 자동 U턴하지 않음. 실기 검증 필요 |
| Stage 6 탐색/복귀 | ⬜ 시작 전 | |
| Stage 7 물체 집기 | ⬜ 시작 전 | |
