...
> 명세는 **계획**이다. 실제 코드를 작성·실기 검증하기 전까지 모든 명세의 상태는 `DRAFT`다.
> 명세대로 구현이 끝나고 실기 Done 이면 해당 스테이지를 [../../PROGRESS.md](../../PROGRESS.md)
> 에 반영한다. 명세와 실제 구현이 어긋나면 **명세를 고쳐** 최신으로 유지한다.
...
| [stage3_node_detect.md](stage3_node_detect.md) | 노드(분기) 감지, `node_advance`, 실패#1 대응 |
...
- BACK 버튼 즉시 정지, 네트워크 비차단(snapshot) 포함.
