...
> 상태: DRAFT (실기 미검증)
> 선행: Stage 1(라인추종) · Stage 2(회전) · Stage 3(노드 감지) 실기 Done.
...
- **하는 것**: 하위 스테이지를 **하나의 주행 루프로 연결**한다.
  선 추종(Stage 1) → 노드 감지(Stage 3) → **미리 정한 시퀀스대로 회전**(Stage 2)
...
- 재사용(수정 금지):
  - Stage 1 `follow`(PID), Stage 3 `follow_to_node`(노드에서 멈춤, bits/dist_mm),
...
```python
follow_to_node()            # Stage1+Stage3 재사용: 노드에서 정지, Arrival(kind,bits,dist_mm)
...
(kp/kd/base_speed/target_reflect=Stage1, turn_*_factor/turn_speed=Stage2, node threshold/
`node_confirm_ms`/`node_advance`=Stage3, 색값=Stage4 — 전부 config/ 의 검증값으로 묻음.)
새로 노출하는 건 **연결부 타이밍**뿐:
...
- [ ] 하위 스테이지 함수 import/재사용 경로 확정(수정 금지): follow_to_node(1+3),
      turn/_pivot/PivotTracker(2), read_node_color_at_rest/classify(4).
