#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""run_maze_v11.py — v4 기반 안정화 실험판(PD + 후보 직진확정 + 색마커 즉시 U턴).

v11 변경점(2026-07-07, 사용자 요청):
  - 주행값 시드 변경: base_speed=16, kp=0.2, turn_speed=6,
    node_confirm_ms=90, node_advance_mm=20, goal_advance_mm=20,
    turn_90_factor=0.66, turn_180_factor=0.71, grip_speed=50 등.
  - PD 조향은 D=0.05 를 이 파일 안에 명시하고, D항 스파이크를 줄이기 위해
    derivative EMA + 상한을 둔다. 회전/후진/일시정지 뒤에는 reset 한다.
  - 브랜치 의심 bits 가 보이면 라인트레이싱(PD)을 끄고 양 바퀴 같은 저속으로
    짧게 직진한 뒤 완전 정지 상태에서 bits 를 다시 읽어 회전/U턴을 결정한다.
    후보 확정 시간 동안 옆 선을 따라가며 대각 자세가 되는 현상을 줄이기 위함이다.
  - 메인 주행 중 색센서가 빨강/노랑/초록을 읽으면 즉시 정지 → U턴 →
    총 2회 부저(회전 완료음 1회 + 추가 1회)를 우선 처리한다.

아래 v4 설명은 탐색 상태머신의 원래 의도를 남긴 것이다.
...
핵심 불변식 (노드 식별이 전역 위치인식 없이 성립하는 이유):
  ...
  **인접 분기 간 이동**이다 — 그 사이 경로에는 커브만 있다(발견 당시 PROBE 가 커브만
  ...
  가 모든 전이에서 보장된다. v4 판단층 테스트의 코스 시뮬레이션이
...
v1/v2/v3 재사용(import — 확정 코드 미수정):
  ...
  마커 색(방문=빨강/도착=초록). v3/v4: 구조와 탐색 상태머신을 따른다.

규약: Python 3.5 안전(f-string 금지) / ev3dev2 는 run() 안 import /
      BACK 버튼 미사용, 정지는 네트워크 stop(robotctl/대시보드) 또는 Ctrl-C.

독립 실행(브릭):  python3 stages/run_maze_v11.py
문법 점검(PC):    python3 -m py_compile stages/run_maze_v11.py lib/*.py
"""
...
# =====================================================================
# 라이브 params — v4 12개 + 노드 판정 좌/우 임계값 2개.
# (탐색 우선순위와 D항/후보 확정 속도는 파일 상수로 둔다.)
# =====================================================================
...
INITIAL_PARAMS = {
    ...
    "left_th_steer": 66,      # 후진 복구 line_found 감도
    "right_th_steer": 63,
    "left_th_node": 18,       # 노드 판정용 좌센서 임계값
    "right_th_node": 14,      # 노드 판정용 우센서 임계값
    "node_advance_mm": 20,    # 확정 후 재판정/회전 전 전진량
    "goal_advance_mm": 20,    # 도착 시퀀스용 전진/후진 거리(호환 유지)
    "turn_90_factor": 0.66,
    "turn_180_factor": 0.71,
    "grab_dist_cm": 6.0,      # ★ 조립에 따라 실기값 다름
    "grip_speed": 50,         # ★ 조립에 따라 부호 반전
}
...
SAVE_PATH = os.path.join(_ROOT, "config", "run_maze_v11.json")
STAGE_NAME = "run_maze_v11"
...
# v11 조향/후보 확정 상수. 라이브 params 를 늘리지 않기 위해 실기 1차값은 파일 상수로 둔다.
PD_KD = 0.05
PD_TURN_LIMIT = 16
PD_D_EMA_ALPHA = 0.35
...
class V11PdController(object):
    """좌/우 raw 반사광 기반 PD. D=0.05, D항은 EMA/상한으로 재포착 흔들림을 줄인다."""
...
def attach_v11_hardware(hw):
    """HoJeongDu 쪽 run_maze 계열이 기대하는 gripper/ultrasonic 별칭을 v11 안에서만 보강."""
...
def run():
    ...
    def handle_marker_color(color, context):
        ...
        log.log("MARKER_UTURN", "COLOR_{}_IMMEDIATE".format(name.upper()),
                ...
                visits=state["visits"], mode=ex.mode)
    ...
    def confirm_node_slow(first_bits, snap):
        ...
        log.log("NODE_CANDIDATE", "PD_OFF_SLOW_STRAIGHT",
                ...
                speed=NODE_CONFIRM_SPEED, phase=phase())
        ...
        while (time.monotonic() - start) < confirm_s:
            ...
            _publish(tele, params, started, mode="node_confirm_slow",
                     ...
                     color=color, bits=bits_to_str(bits))
            time.sleep(LOOP_DELAY_MS / 1000.0)
    ...
    print("run_maze_v11 ready. waiting YELLOW on center sensor... "
    ...
    while hw.read_center_color_value() != COL_YELLOW:
        if stop_flag["on"]:
            ...
            server.stop()
            print("run_maze_v11 stopped before start.")
            return
        action = take_pending()
    ...
    print("run_maze_v11 running. stop via 'robotctl stop' or Ctrl-C.")
    try:
        while not state["done"]:
            ...
            # (4) 노드 후보: PD를 끄고 저속 직진-정지 재판정으로 확정한다.
